...
class_names = class_names_.tolist()
print(class_names_)
print(class_names)
...
def gen_reports(mode='val'):
    ...
    file_rep.write('\n---------------')
    file_rep.write('\nClass names:\n')
    file_rep.write(str(class_names))
    file_rep.write('\nConfusion matrix:\n')
    ...
    fig_obj = plt.figure()
    metrics.ConfusionMatrixDisplay(conf_mat, display_labels=class_names).plot()
    # Save figure in disk
...
info = f'{args.ec};{args.ds};{args.arch};{args.optim};{args.ft};{args.datrain};{args.daval};{args.datest};' + \
       ...
       f'{mm_str};{ss_str};' + \
       f'{args.ep};{es_};' + \
       f'{acc_val};{acc_test};{train_acc_};{val_acc_};' + \
